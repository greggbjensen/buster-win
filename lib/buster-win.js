...
(function(){
    ...
    var url = require('url');
    var router = require('./buster-router');
    ...
    var _createHttpServer =  function (dir, config) {
        ...
        var rootPath = config.fullRootPath;
        ...
        router.initializeRoutes(config, { scriptFiles: scriptFiles });
        ...
        return http.createServer(function (req, res) {
            ...
            var matchedRoute = router.matchRoute(urlInfo.pathname);
            if(matchedRoute) {
                //check for route parameters and add key/values to request
                req.params = router.extractParams(urlInfo.pathname, matchedRoute);
                req.query = router.extractQuery(urlInfo.query)
    ...
    busterWin.execute = function(dir) {
        ...
        _(configList).each(function(config, name){
            ...
            // Set file system root path into config.
            if (config.rootPath) {
                config.fullRootPath = path.join(dir, config.rootPath || '');
            }

            // If there are any extensions, execute them.
            if (config.extensions) {
                config.extensions.forEach(function(extension){
                    extension.create({});
                    extension.configure({ options: config });
                });
            }
