...
(function(){
    ...
    var _executeBrowserTests = function(dir, config) {
        ...
        _simulateBrowser(config, function(code){
            ...
            // Close server unless requested to stay open.
            if (!config.stayOpen) {
                server.close();
            };
        });
    ...
    var _simulateBrowser =  function(config, done){
        ...
        var phantom = spawn('phantomjs', [phantomRunner, 'http://localhost:' + config.port]);
    ...
    var _createHttpServer =  function (dir, config) {
        ...
        return http.createServer(function (req, res) {
            ...
            // Standard file loading.
                ...
                else if (/^\/buster-win/.test(req.url)) {
                    filePath = path.join(__dirname, '../script', req.url.replace(/^\/buster-win/, ''));
                }
                // Standard path.
                else {
