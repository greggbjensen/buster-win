...
(function(){
    ...
    // Supply all color functions.
    var colors = ['green', 'cyan', 'grey', 'yellow', 'red', 'purple'];
    for (var i = 0; i < colors.length; i++) {
        term[colors[i]] = term.color;
    }

    // Alias console reporter.
    var consoleReporter = buster.reporters.console;

    buster.reporters.dots = buster.extend(buster.create(consoleReporter), {
        create: function (opt) {
            opt = opt || {};
            var reporter = buster.create(this);
            reporter.io = opt.io || io;
